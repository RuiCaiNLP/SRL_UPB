...
def get_batch(input_data, batch_size, word2idx, fr_word2idx, lemma2idx, pos2idx, pretrain2idx, fr_pretrain2idx,
    ...
    for batch_i in range(int(math.ceil(len(input_data)/batch_size))):
        ...
        if lang=='En':
            word_batch = None#[[word2idx.get(item[6],word2idx[_UNK_]) for item in sentence] for sentence in data_batch]
            pad_word_batch = None#np.array(pad_batch(word_batch, batch_size, word2idx[_PAD_]))
        else:
            word_batch = None#[[fr_word2idx.get(item[6], fr_word2idx[_UNK_]) for item in sentence] for sentence in data_batch]
            pad_word_batch = None#np.array(pad_batch(word_batch, batch_size, fr_word2idx[_PAD_]))
        ...
        if lang=='En':
            pretrain_word_batch = None#[[pretrain2idx.get(item[6], pretrain2idx[_UNK_]) for item in sentence] for sentence in data_batch]
            pad_pretrain_word_batch = None#np.array(pad_batch(pretrain_word_batch, batch_size, pretrain2idx[_PAD_]))
        else:
            pretrain_word_batch = None#[[fr_pretrain2idx.get(item[6], fr_pretrain2idx[_UNK_]) for item in sentence] for sentence in
                                  # data_batch]
            pad_pretrain_word_batch = None #np.array(pad_batch(pretrain_word_batch, batch_size, pretrain2idx[_PAD_]))
